...
def parse_detail(html,url):
    alllog.logger.info("黑龙江省药品监督管理局: %s"%url)
    doc=pq(html)
    data={}
    ...
    data["classification"]="黑龙江省药品监督管理局"
    data["url"]=url
    print(data)
    save(data)
...
def main():
    for i in range(0,9):
        print(i)
        if i==0:
            url="http://zizhan.mot.gov.cn/st/hebei/tongzhigonggao/index.html"
        else:
            url="http://zizhan.mot.gov.cn/st/hebei/tongzhigonggao/index_"+str(i)+".html"
        html=get(url)
        parse_index(html)
