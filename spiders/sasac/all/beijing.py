...
def parse_detail(html,url):
    alllog.logger.info("北京国有资产委员会: %s"%url)
    doc=pq(html)
    data={}
    data["title"]=doc(".details_page p").text()
...
def parse_index(html):
    ...
    for item in items:
        url=item.attr("href")
        if "http" not in url:
            url="http://gzw.beijing.gov.cn/xxfb/zcfg" + url.replace("./","/")
        try:
            html=get(url)
        except:
...
def main():
    for i in range(0,6):
        ...
        if i==0:
            ...
            url="http://gzw.beijing.gov.cn/xxfb/zcfg/index_"+str(i)+".html"
        html=get(url)
        parse_index(html)
