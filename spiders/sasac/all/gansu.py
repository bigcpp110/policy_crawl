...
def parse_detail(html,url):
    alllog.logger.info("甘肃省国有资产委员会: %s"%url)
    doc=pq(html)
    data={}
    ...
    data["classification"]="甘肃省国有资产委员会"
    data["url"]=url
    print(data)
    save(data)
...
def parse_index(html):
    doc=pq(html)
    items=doc(".UlTab li a").items()
    for item in items:
        url=item.attr("href")
        if "http" not in url:
            url="http://gzw.gansu.gov.cn:81" + url.replace("\\","/")
        try:
            html=get(url)
        except:
...
def main():
    for i in range(1,17):
        print(i)
        url="http://gzw.gansu.gov.cn:81/html/71list-"+str(i)+".html"
        html=get(url)
        parse_index(html)
