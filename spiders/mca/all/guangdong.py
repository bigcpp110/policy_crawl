...
def parse_detail(html,url):
    alllog.logger.info("广东省民政厅: %s"%url)
    doc=pq(html)
    data={}
...
def main():
    for i in range(1,13):
        print(i)
        if i==1:
            url="http://smzt.gd.gov.cn/zwgk/zcfg/index.html"
        else:
            url="http://smzt.gd.gov.cn/zwgk/zcfg/index_"+str(i)+".html"
        html=get(url)
        parse_index(html)
