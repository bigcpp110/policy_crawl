...
def parse_detail(html,url):
    alllog.logger.info("广东省教育厅: %s"%url)
    doc=pq(html)
    data={}
...
def main():
    for i in range(1,21):
        print(i)
        if i==1:
            url="http://edu.gd.gov.cn/zxzx/tzgg/index.html"
        else:
            url="http://edu.gd.gov.cn/zxzx/tzgg/index_"+str(i)+".html"
        html=get(url)
        parse_index(html)
